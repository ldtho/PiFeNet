...
def get_image_path(idx, prefix, training=True, relative_path=True, exist_check=True):
    tail = '.jpg' if "jrdb" in prefix.lower() else '.png'
    return get_kitti_info_path(idx, prefix, 'image_2', tail, training,
...
def get_velodyne_path(idx, prefix, training=True, relative_path=True, exist_check=True):
    # tail = '.pcd' if "jrdb" in prefix.lower() else '.bin'
    tail = '.bin'
    return get_kitti_info_path(idx, prefix, 'velodyne', tail, training,
                               relative_path, exist_check)
...
def get_kitti_image_info(path,
    ...
    """
    # jrdb = True if 'jrdb' in path.lower() else False
    ...
    def map_func(idx):
        info = {}
        # pc_info = {'num_features': 4 if not jrdb else 3}
...
def get_label_anno(label_path):
    ...
    annotations['dimensions'] = np.array(
        ...
        -1, 3)[:, [2, 0, 1]] # h,w,l KITTI cam -> l,h,w KITTI cam == whl LIDAR
