...
def kitti_anno_to_label_file(annos, folder, filter_classes=["Pedestrian", "Cyclist"]):
    folder = Path(folder)
    c = False
    for anno in annos:
        ...
        for j in range(anno["bbox"].shape[0]):
            if anno["name"][j] not in filter_classes:
                continue
            label_dict = {
                'name': anno["name"][j],
                'alpha': anno["alpha"][j],
...
def _calculate_num_points_in_gt(data_path,
                                ...
                                remove_outside=True,
                                num_features=4):
    jrdb = True if 'jrdb' in data_path.lower() else False
    ...
    for info in tqdm(infos):
        ...
        points_v = np.asarray(o3d.io.read_point_cloud(str(v_path)).points, dtype=np.float32).reshape(
            [-1, num_features])

        if remove_outside and not jrdb:
            calib = info["calib"]
            rect = calib['R0_rect']
            Trv2c = calib['Tr_velo_to_cam']
            P2 = calib['P2']
        ...
        if jrdb:
            gt_boxes_lidar = box_np_ops.box_camera_to_lidar_jrdb(
                gt_boxes_camera)
        ...
        # TODO: clean
        # pc = o3d.io.read_point_cloud(str(v_path))
        # geo = [pc]
        # bboxes = gt_boxes_lidar
        # bbox_corners = box_to_3d_corners(bboxes)
...
def create_kitti_info_file(data_path, save_path=None, relative_path=True, calib=True, dataset='kitti'):
    ImageSets = f"ImageSets_{dataset}"
    ...
    kitti_infos_train = kitti.get_kitti_image_info(
        ...
        training=True,
        velodyne=True,
        calib=True if dataset == 'kitti' else False,
        image_ids=train_img_ids,
        relative_path=relative_path)
    if 'kitti' in dataset:
        _calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path)
    elif 'jrdb' in dataset:
        _calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path,
                                    remove_outside=False, num_features=4)
    filename = save_path / f'{dataset}_infos_train.pkl'
    print(f"{dataset} info train file is saved to {filename}")
    ...
    if 'kitti' in dataset:
        ...
        _calculate_num_points_in_gt(data_path, kitti_infos_val, relative_path,
                                    ...
                                    num_features=4)
    filename = save_path / f'{dataset}_infos_val.pkl'
    print(f"{dataset} info val file is saved to {filename}")
    with open(filename, 'wb') as f:
        pickle.dump(kitti_infos_val, f)
    filename = save_path / f'{dataset}_infos_trainval.pkl'
    print(f"{dataset} info trainval file is saved to {filename}")
    with open(filename, 'wb') as f:
    ...
    kitti_infos_test = kitti.get_kitti_image_info(
        ...
        relative_path=relative_path)
    filename = save_path / f'{dataset}_infos_test.pkl'
    print(f"{dataset} info test file is saved to {filename}")
    with open(filename, 'wb') as f:
