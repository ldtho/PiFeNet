...
from lyft_dataset_sdk.eval.detection.mAP_evaluation import recall_precision, get_class_names, get_average_precisions
...
def prep_pointcloud(input_dict,
    ...
    if training:
        ...
        # print('difficulty', np.max(points, axis=0))
        # print(points.shape)
        ...
        if db_sampler is not None:
            ...
            sampled_dict = db_sampler.sample_all(
                ...
                calib=calib)
            # print('sampled_dict', np.max(points, axis=0))
            ...
            if sampled_dict is not None:
                ...
                # print('remove_points_after_sample', np.max(points, axis=0))
        ...
        # print('prep.noise_per_object_v3_before', np.max(points, axis=0))
        prep.noise_per_object_v3_(
            ...
            group_ids=group_ids,
            num_try=100)
        # print('prep.noise_per_object_v3_', np.max(points, axis=0))
    ...
    # print('shuffle_points', np.max(points, axis=0))
    # exit()
    # [0, -40, -3, 70.4, 40, 1]
    ...
    # print('create_targetsbefore', np.max(points, axis=0))
    # print('gt_dict["gt_boxes"]', gt_dict["gt_boxes"])
    # print('np.max["gt_boxes"]', np.max(gt_dict["gt_boxes"],axis=0))
    if create_targets:
        t1 = time.time()
        # print('gt_dict["gt_classes"]',gt_dict["gt_classes"])
        # print('gt_dict["gt_names"]',gt_dict["gt_names"])
        # print('anchors',anchors[0])
        # print('anchors_dict',anchors_dict.keys())
        # exit()
        ...
        # print("targets_dict['labels']",targets_dict['labels'])
        # print("targets_dict['bbox_targets']",targets_dict['bbox_targets'])
        # print("targets_dict['importance']",targets_dict['importance'])
        # print(np.min(example['labels']),np.max(example['labels']))
        # print(np.min(example['reg_targets']),np.max(example['reg_targets']))
        # for lab, reg in zip(example['labels'],example['reg_targets']):
        #     if lab>0:
        #         print(lab,reg)
        # exit()
    # print('np.maxexample["reg_targets"]', np.max(example["reg_targets"], axis=0))
    # print('lenexample["reg_targets"]', len(example["reg_targets"]))
    # exit()
    # example['points']= points
    return example
