...
class RPNNoHeadBase(nn.Module):
    def __init__(self,
        ...
        for i, layer_num in enumerate(layer_nums):
            if (i == len(layer_nums) - 1) and mhsa:
                ...
                block, num_out_filters = self._make_layer(
                    ...
                    layer_num,
                    stride=layer_strides[i])
                self._resolution[0] /= layer_strides[i]
                self._resolution[1] /= layer_strides[i]
            blocks.append(block)
...
@register_rpn
class RPNV3(RPNBase):
    def _make_layer(self, inplanes, planes, num_blocks, stride=1, mhsa=False, height=14, width=14):
        if self._use_norm:
            if self._use_groupnorm:
                BatchNorm2d = change_default_args(
                    num_groups=self._num_groups, eps=1e-3)(GroupNorm)
        ...
        height, width = int(height / stride), int(width / stride)
        if not mhsa:
            for j in range(num_blocks):
                block.add(Conv2d(planes, planes, 3, padding=1))
                block.add(BatchNorm2d(planes))
            ...
            for j in range(num_blocks):
                # block.add(MHSA(planes, height, width, heads=4, rel_pos_emb=False))
                block.add(AttentionBlock(n_dims = planes, height=height, width=width,
                                         num_heads=4, use_pos_emb=False))
                # block.add(BatchNorm2d(planes)) #v5 prenorm -fail
            block.add(LayerNorm2d(planes))
            block.add(nn.GELU())
        return block, planes
