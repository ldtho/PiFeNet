...
from second.pytorch.models.MiniBiFPN import MiniBiFPN
...
class VoxelNet(nn.Module):
    def __init__(self,
        ...
        if vfe_class_name == 'PillarFeatureDANet':
            self.voxel_feature_extractor = voxel_encoder.get_vfe_class(vfe_class_name)(
                ...
                boost_channel_dim=boost_channel_dim,
                reduction_rate=reduction_rate,
                num_point_per_voxel=num_point_per_voxel,
                channel_att=channel_att,
                point_att=point_att,
                task_aware_att=task_aware_att,
        ...
        if rpn_class_name in rpn_dict.keys():
            self.rpn = rpn_dict[rpn_class_name](num_blocks=rpn_layer_nums,
                                                mid_planes=rpn_num_filters,
                                                num_class=num_class,
                                                resolution=[int(output_shape[2]), int(output_shape[3])],
                                                strides=rpn_layer_strides,
                                                upsample_strides=rpn_upsample_strides,
                                                num_upsample_filters=rpn_num_upsample_filters,
                                                num_input_filters=rpn_num_input_features,
                                                num_anchor_per_loc=target_assigner.num_anchors_per_location,
                                                encode_background_as_zeros=encode_background_as_zeros,
                                                use_direction_classifier=use_direction_classifier,
                                                box_code_size=target_assigner.box_coder.code_size,
                                                num_direction_bins=self._num_direction_bins)
        # elif rpn_class_name == 'RPNV3':
    ...
    def predict(self, example, preds_dict):
        ...
        for box_preds, cls_preds, dir_preds, a_mask, meta in zip(
            ...
            # direction hack?
            # selected_boxes[..., 6] += np.pi
            # print("BEFOREEEE")
            # print(selected_boxes)
            # print(selected_boxes.min(), selected_boxes.max())
            ...
            if selected_boxes.shape[0] != 0:
                ...
                # print("AFTERRRRRR")
                # print(box_preds)
                # print(box_preds.min(), box_preds.max())
                # print("=====================================")
    ...
    def update_metrics(self, cls_loss, loc_loss, cls_preds, labels, sampled):
        ...
        for i, thresh in enumerate(self.rpn_metrics.thresholds):
            ret["pr"][f"prec_{int(thresh * 100)}"] = float(prec[i])
            ret["pr"][f"rec_{int(thresh * 100)}"] = float(recall[i])
        return ret
