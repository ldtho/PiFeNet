...
def _create_learning_rate_scheduler(learning_rate_config, optimizer, total_step):
    ...
    if learning_rate_type == 'exponential_decay':
        ...
        lr_scheduler = lsf.ExponentialDecay(
            optimizer, total_step, config.initial_learning_rate, config.decay_length, config.decay_factor,
            config.staircase)
    ...
    # if learning_rate_type == 'cosine_anneal_warmup':
    #     config = learning_rate_config.cosine_anneal_warmup
    #     lr_scheduler = CosineAnnealingWarmupRestarts(
    #         optimizer, first_cycle_steps=config.first_cycle_steps, cycle_mult=config.cycle_mult,
    #         max_lr = config.max_lr, min_lr=config.min_lr,warmup_steps=config.warmup_steps,gamma=config.gamma
    #     )
